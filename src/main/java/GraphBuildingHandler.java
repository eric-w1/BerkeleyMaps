...
//            System.out.println("Tag with k=" + k + ", v=" + v + ".");
            ...
            g.nodeMap.get(lastNode).name = attributes.getValue("v");
...
//            System.out.println("Finishing a way...");
            if (ALLOWED_HIGHWAY_TYPES.contains(highway)) {
                for (int i = 0; i < nodes.size() - 1; i += 1) {
                    g.addEdge(nodes.get(i), nodes.get(i + 1), roadname, highway);
                }
            }
        }
        if (qName.equals("node")) {
            GraphDB.Node p = g.nodeMap.get(lastNode);
            if (p.name != null) {
                List<GraphDB.Node> matches = g.trie.get(g.cleanString(p.name));
                if (matches == null) {
                    List<GraphDB.Node> matchingNodes = new ArrayList<>();
                    matchingNodes.add(p);
                    g.trie.put(g.cleanString(p.name), matchingNodes);
                } else {
                    matches.add(p);
                }
            }
